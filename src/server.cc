...
void log(char c, const char *format, ...) {
    ...
    if (isatty(1)) {
        ...
        default:
            ...
            cc[1] = 0;
        case '.':
            strcpy(cc, "\x1b[35m.\x1b[0m");
            break;
        case '*':
            strcpy(cc, "\x1b[1m*\x1b[0m");
            break;
        case '#':
            strcpy(cc, "\x1b[33m#\x1b[0m");
            break;
        }
    } else {
        cc[0] = c;
    ...
    fprintf(stderr, "%d:M %s.%d %s %s\n", getpid(), tbuf, ((int)(spec.tv_nsec / 1.0e6)) % 1000, cc,
            buffer);
    va_end(args);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0 ||
            strcmp(argv[i], "-?") == 0) {
            fprintf(stdout, "RocksDB version " ROCKSDB_VERSION ", Libuv version " LIBUV_VERSION
    ...
    log('#', "Server started, RocksDB version " ROCKSDB_VERSION ", Libuv version " LIBUV_VERSION
             ", Server version " SERVER_VERSION);
    opendb();
